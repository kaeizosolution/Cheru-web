...
class Product extends MY_Controller {
    ...
    function post($product_id = NULL)
    {
		...
		###
        $selected_variation_obj = array(); $variation_obj_keys = array();
        ...
        if($_POST){
            ...
            if($product_id){
				...
				{
					foreach($selected_variation_obj as $row_wise)
					{
						$all_attr = str_replace(',','',$row_wise->attribute_item_id);
						$row_wise->all_attr = $all_attr;
						$variation_obj_keys[$all_attr][] = $row_wise;		
					}
				}
            }
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['variation_obj_keys'] 		= $variation_obj_keys;
...
//https://stackoverflow.com/questions/44918503/uploading-multiple-images-with-crop-images-in-codeigniter-but-getting-too-much
            ...
            if(isset($product_id)){
                ...
                if(!empty($_POST['_regular_price'])){
                    ...
                    foreach($_POST['attr_itemid'] as $index => $value){
                        $_thumbnail_id = NULL;
						$attr_key_all = str_replace(',', '', $value);
						$thumbnail_row_wise = '_thumbnail_id_'.$attr_key_all;
						if(count($_FILES[$thumbnail_row_wise]['name'])){
						for ($i = 0; $i < count($_FILES[$thumbnail_row_wise]['name']); $i++) {
                            $_FILES['file']['name']     = $_FILES[$thumbnail_row_wise]['name'][$i];
                            $_FILES['file']['type']     = $_FILES[$thumbnail_row_wise]['type'][$i];
                            $_FILES['file']['tmp_name'] = $_FILES[$thumbnail_row_wise]['tmp_name'][$i];
                            $_FILES['file']['error']    = $_FILES[$thumbnail_row_wise]['error'][$i];
                            $_FILES['file']['size']     = $_FILES[$thumbnail_row_wise]['size'][$i];
						
                            $thumbnail = $this->upload_color_image($_FILES);
                        ...
                        $post_variation = array(
                            ...
                            '_thumbnail_id' => $_thumbnail_id[$i],
							//'_sku' => $sku,
							'_stock' => $stock,
							);
						#print_r($post_variation); 
    ...
    public function upload_color_image($FILES)
    {
        ...
        if($FILES['file']['name'])
        {
            ...
            if($this->upload->do_upload('file'))
            {
                ...
                $config['new_image']       = "./assets/uploads/".$fileData['file_name'];
                $config['create_thumb']    = TRUE;
                $config['maintain_ratio']  = TRUE;
                $config['width']           = 800;
                $config['height']          = 800;
            ...
            {
                print "UPLOAD ERROR: " . $this->upload->display_errors();
                return false;
            }
        }
    }
	...
	public function export(){ 
		...
		$list = $this->Product_model->get_export();		
