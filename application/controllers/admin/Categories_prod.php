...
class Categories_prod extends MY_Controller {
    ...
    public function __construct(){
        ...
        $this->check_module_permission('categories_prod');
    }
    ...
    public function index(){
        ...
        $crumbs = array($page_lang->home => "/$this->TYPE/dashboard", "Category Listing" => ""); 
    ...
    function multilevel_child($data, &$results, $hyphen='')
    {
        ...
        foreach($data as $menu)
        {
            ...
            $row['slug'] = $menu->slug;
            $row['status'] = (string)(int)$menu->status;
            $row['status_label'] = isset($type_status[$menu->status]) ? $type_status[$menu->status] : 'Inactive';
            $row['id'] = $menu->id;
            $results[] = $row;
            if(!empty($menu->sub))
            {
    ...
    public function index_ajax_cat()
    {
        ...
        $rs = isset($new_array[$start]) ? $new_array[$start] : [];
        $output = array(
    ...
    public function add($id = NULL){
		...
		$parent_id_post = ($this->input->post('parent_id')) ? $this->input->post('parent_id') : 0;
		if((int)$parent_id_post === 0 && isset($_POST) && $_POST){
        ...
        if($this->form_validation->run() == true){
			...
			// icon upload removed from UI; keep backend tolerant if a file is posted
			if(isset($_FILES['icon']) && !empty($_FILES['icon']['name'])){				
				...
				$icon = isset($icon_img['file_name']) ? $icon_img['file_name'] : '';
			}
            ...
            $statusVal = ($this->input->post('status') !== null) ? (int)$this->input->post('status') : 1;
            $category = array(
                    'name' => $this->input->post('name'),
    ...
    public function update($id = null){
        ...
        $data = array();
        $crumbs = array("Home" => "/$this->TYPE/dashboard", "Category Listing" => "/$this->TYPE/categories_prod", "Edit Category" => ''); 
        $breadcrumbs = $this->breadcrumbs->show($crumbs);
        $data['breadcrumbs']    = $breadcrumbs;
        ...
        $all_parents = $this->Product_model->get_root_parent($id);
        $all_parents_cnt = count($all_parents) > 3 ? 3 : count($all_parents);
        		$column_name = array('1'=>'category_id', '2'=>'sub_cat_id', '3'=>'sub_cat_id');
        ...
        if($this->form_validation->run() == true){
            ...
            $statusVal = ($this->input->post('status') !== null) ? (int)$this->input->post('status') : 0;
            $category = array(
                    'name' => $this->input->post('name'),
                    'name_es' => $this->input->post('name_es'),
					'slug' => $cat_slug,
            ...
            if($statusVal == 0)
            {
                $this->Query_model->update_data('ec_product', array('status'=>'0'), array($column_name[$all_parents_cnt] => $id));
            }
