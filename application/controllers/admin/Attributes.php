...
class Attributes extends MY_Controller {
	...
	function __construct()
	{
        ...
        $this->check_module_permission('attributes');
    }
	
	public function index()
    {
        $page_lang = get_page_language_data('admin_page_lang');
        $crumbs = array($page_lang->home => "/$this->TYPE/dashboard", $page_lang->attributes => "");
        ...
        foreach ($list as $obj) {
            ...
            if($item_obj){
                foreach($item_obj as $it){
                    $item_str.= "<a href=\"/admin/attributes/update_attribute_item/$obj->attribute_id/$it->attribute_item_id\">$it->name</a>&nbsp;&nbsp;";
                }
            }
            $item_str.= "<br><a href=\"/admin/attributes/add_attribute_item/$obj->attribute_id\" class=\"table_btn_add\">Add</a>";
            $row['attribute_id']= $obj->attribute_id;
            $row['master']      = $obj->master;
            $row['name']        = $obj->name;
            $row['slug']        = $obj->slug;
            $row['item']        = $item_str;
	...
	function post($attribute_id = NULL)
    {
		...
		$this->form_validation->set_rules('name', 'name', 'trim|required|xss_clean|callback_unique_field['.$attribute_id.']');
        ...
        if($_POST) {
            ...
            if(isset($attribute_id)){
                $data =  array(
                    ...
                    'name_es'       => $attributes_obj->name_es,
        ...
        if ($this->form_validation->run() == FALSE){
            $page_lang = get_page_language_data('admin_page_lang');
            $action_bc = isset($attribute_id) ? $page_lang->update : $page_lang->add;
            $crumbs = array($page_lang->home => "/$this->TYPE/dashboard", $page_lang->attributes => "/$this->TYPE/attributes", "$action_bc" => "action");
            $breadcrumbs = $this->breadcrumbs->show($crumbs);
            $data['breadcrumbs']    = $breadcrumbs;
            ...
            $data['csrf'] = csrf_token();
            $data['TYPE'] = $this->TYPE;
            $data['action'] = isset($attribute_id) ? $page_lang->update : $page_lang->add;
	...
	function post_attribute_item($args)
    {
		...
		if($attribute_item_id){
			$this->form_validation->set_rules('name', 'Name', 'trim|required|xss_clean|callback_unique_item['.$attribute_item_id.']');
		}else{
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            if($attribute_item_id){
				...
				$names = array_map('trim', explode(',', $raw_name));
				$names = array_values(array_filter($names, function($n){ return $n !== ''; }));

				$inserted = 0;
				$skipped = 0;
				foreach ($names as $nm) {
					$exists = $this->Query_model->get_data_obj('ec_attribute_item', array('attribute_id' => $attribute_id, 'name' => $nm, 'status' => array('0','1')));
					if ($exists) {
						$skipped++;
						continue;
					}
	...
	function unique_field($value, $attribute_id)
    {
        $name = $this->input->post('name');
        ...
        $supplier_obj = $this->Query_model->get_data_obj('ec_attribute',$args);

        $status   = TRUE;
        $message  = '';
        if($supplier_obj) {
            $status   = FALSE;
            $message  = 'Attribute name must be unique <br/>';
	...
	function unique_item($value, $attribute_item_id)
    {
        
		$name = $this->input->post('name');
		$attribute_id = $this->uri->segment(4); 
        $args = array('name' => $name,'attribute_id' => $attribute_id);
        if($attribute_item_id){
            $args['attribute_item_id!='] = $attribute_item_id;
        }
        $attributeitem_obj = $this->Query_model->get_data_obj('ec_attribute_item',$args);
		$status   = TRUE;
        $message  = '';
        if($attributeitem_obj) {
            $status   = FALSE;
            $message  = 'Attribute Item name must be unique <br/>';
            $this->form_validation->set_message('unique_item', $message);
        }
