...
class User extends MY_Controller {
	...
	function __construct()
	{
        ...
        $this->check_module_permission('customer');
    }
	...
	public function index_ajax_post()
    {
        ...
        $list = $this->User_model->get_order_datatable();		
        $data = array();
		...
		foreach ($list as $obj) {					
			...
			$st = (string)((int)($customer_data->status ?? $obj->status ?? 0));
            $row = array();
			$row['customer_id']   		= $obj->customer_id;
			$row['fname']   			= ($customer_data->fname) ? $customer_data->fname.' '.$customer_data->lname : 'NA';
            $row['email']      			= ($customer_data->email) ? $customer_data->email : 'NA';
			$row['mobile']     			= $customer_data->mobile;
			$row['wallet_balance']		= isset($obj->wallet_balance) ? $obj->wallet_balance : 0;
			
			$pref_cur_id = isset($customer_data->preferred_currency_id) && $customer_data->preferred_currency_id > 0 ? (int)$customer_data->preferred_currency_id : 1;
			$pref_cur = get_currency($pref_cur_id);
			$row['preferred_currency']  = $pref_cur ? $pref_cur->iso_code : 'N/A';

			$row['order_date']     	 	= '-';//date('Y-m-d', strtotime($obj->date_created));		
        ...
        $output = array(
                ...
                "recordsTotal" => $this->Query_model->count_all('ec_customer'),
	...
	public function view()
    {
		...
		foreach ($shipping_address as $val) 
        {
            $row = array();
			$shipping_first_name = '';
			if (isset($val->fullname) && $val->fullname) {
				$shipping_first_name = (string)$val->fullname;
			} elseif (isset($val->fname) || isset($val->lname)) {
				$shipping_first_name = trim(((string)($val->fname ?? '') . ' ' . (string)($val->lname ?? '')));
			}
	...
	public function status_change(){
		...
		$status_str = strtolower(trim((string)$status));
		$status_val = in_array($status_str, array('1', 'active'), true) ? 0 : 1;
		$data['csrf'] = csrf_token();
        $data = array();
		// Detect whether ec_customer.status is numeric (0/1) or string (active/inactive)
		$master = $this->load->database('master', TRUE);
		$sample = $master->select('status')->from('ec_customer')->where('customer_id', (int)$id)->get()->row();
		$sample_status = $sample ? (string)$sample->status : '';
		$db_status_val = is_numeric($sample_status) ? (string)((int)$status_val) : ((int)$status_val === 1 ? 'active' : 'inactive');

		$update = $this->User_model->status_change($id, $db_status_val);
	...
	public function checkall_status_change(){
		...
		$status_str = strtolower(trim((string)$status));
		$status_val = in_array($status_str, array('1', 'active'), true) ? 1 : 0;
		if (!is_array($ids)) {
			$ids = array();
		}
		$ids = array_values(array_filter(array_map('intval', $ids)));
		$update = false;
		if(!empty($ids) && $this->LOGIN_ID && $this->TYPE == 'admin'){
			// Use master DB explicitly (writes). Listing reads from slave DB.
			$master = $this->load->database('master', TRUE);
			$sample = $master->select('status')->from('ec_customer')->where('customer_id', (int)$ids[0])->get()->row();
			$sample_status = $sample ? (string)$sample->status : '';
			$db_status_val = is_numeric($sample_status) ? (string)((int)$status_val) : ((int)$status_val === 1 ? 'active' : 'inactive');
			$master->where_in('customer_id', $ids);
			$update = (bool)$master->update('ec_customer', array('status' => $db_status_val));
		}
		echo json_encode(array('success' => $update));
	}
