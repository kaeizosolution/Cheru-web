...
class Hot_new_arrivals extends API_Controller
{
    ...
    public function index()
    {
        ...
        if (function_exists('current_currency') && function_exists('get_currency')) {
            $cc_hna = current_currency();
            $cur_hna = $cc_hna ? get_currency($cc_hna) : null;
            if ($cur_hna && isset($cur_hna->rate) && is_numeric($cur_hna->rate)) {
                $cur_rate_hna = (float)$cur_hna->rate;
            }
        }
        try {
            $since = date('Y-m-d H:i:s', strtotime('-30 days'));
            $rows = $this->db
                ->select('p.id, p.name, p.date_added, p.vendor_id, p.product_type')
                ->from('products p')
                ->join('ec_vendor v_check', 'v_check.vendor_id = p.vendor_id', 'left')
                ->where('p.status', '1')
                ->where('p.date_added >=', $since)
                ->group_start()
                    ->where('p.vendor_id', 0)
            ...
            if (is_array($rows) && $rows) {
                foreach ($rows as $p) {
                    ...
                    if ($minp && isset($minp->price)) {
                        $min_price = (float)$minp->price * $cur_rate_hna;
                    }
        ...
        return $this->ok(['hot_new_arrivals' => $out], 'success');
    }
}
