...
<style>
    ...
    .custom-popover-btn:hover { background: #2563eb; color: #fff !important; }
    .custom-popover-btn i { color: #fff !important; } /* Force white icon */
...
<div class="page-body">
    <div class="container-fluid">
        <div class="page-header">
            <div class="row">
                <div class="col-lg-6">
                    <h3 style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1e293b;"><?= $page_lang->shipping_panel; ?></h3>
                </div>
                <?= $breadcrumbs ?>
            </div>
        </div>
            ...
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-header-custom">
                        <h5 class="card-title-custom">Shipping List</h5>
                        <div class="topAction-btn" style="display:flex; gap:10px;">
                            <a href="/<?= $TYPE?>/driver/add_shipping" class="btn-custom btn-add" title="<?= $page_lang->add_new; ?>">
                                <i class="fa fa-plus"></i> Add New
                            </a>
                            <a href="javascript:void(0)" data-toggle="modal" data-target="#myModal" class="btn-custom btn-filter" title="<?= $page_lang->filter; ?>">
                    ...
                    <div class="card-body p-0">
                        <div class="dt-ext table-responsive">
                            <table id="table" class="display dataTable custom-table">
                                <thead>
                                    <tr>
                                        <th><?= $page_lang->shipping; ?></th>
                                        <th><?= $page_lang->start_date; ?></th>
                                        <th><?= $page_lang->end_date; ?></th>
                                        <th><?= $page_lang->price_per_km; ?></th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold">Filter Shipping</h5>
...
function dataTable()
{
    ...
    var filterData = {};
    if($('#apply_date').val()) { filterData.apply_date = $('#apply_date').val(); }
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "/<?= $TYPE; ?>/driver/index_ajax_shipping",
            "data": postData,
            "type": "POST",
            "dataType": 'json',
            "complete": function () {
        ...
        "columnDefs": [ 
            ...
            },
            {
                ...
                "render": function ( data, type, row ) {
                    // FIX: Clean string structure for popover content
                    var editBtn = '<button class="btn custom-popover-btn btn-edit" type="button" data-id="'+row.price_id+'">' +
                                  '<i class="fa fa-pencil-square-o"></i> Edit</button>';
                    
                    var actionTemplate = '<button type="button" class="btn btn-premium-action" data-toggle="popover" ' +
                                       'data-placement="bottom" data-html="true" data-content=\'' + editBtn + '\'>' +
...
$(document).on('click','.btn-edit', function(e){
    ...
    var url="/<?php echo $TYPE ?>/driver/update_shipping/"+uid+"";
    url_new_tab(e,url);
});
...
$(document).on('click','.btn-premium-action', function(e){
    ...
    $(this).popover('toggle');
    // Note: No need to init tooltip inside popover if we just use a button with text
});
</script>