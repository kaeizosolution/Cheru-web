...
<div class="page-body">
    <div class="container-fluid">
        <div class="page-header">
            <div class="row">
                <div class="col-lg-6">
                    <h3 style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1e293b;"><?= $page_lang->payment; ?></h3>
                </div>
                <?= $breadcrumbs ?>
            </div>
        </div>
            ...
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-header-custom">
                        <h5 class="card-title-custom">Payment List</h5>
                        <div class="topAction-btn" style="display:flex; gap:10px;">
                            <a href="javascript:void(0)" data-toggle="modal" data-target="#payment" class="btn-custom btn-filter" title="Filter">
                    ...
                    <div class="bulk-actions-bar">
                        <select class="form-control-sm-custom btn-checkall" id="selectStatus" name="checkbox_all">
                            <option value=""><?= $page_lang->status; ?></option>   
                            <option value="1"><?= $page_lang->active; ?></option>                          
                            <option value="0"><?= $page_lang->inactive; ?></option>   
                        </select>
                        <button class="btn-custom btn-submit-bulk" type="button" onclick="checkAll()">
                            <?= $page_lang->submit; ?>
                        </button>
                    </div>
                    ...
                    <div class="card-body p-0">
                        <div class="dt-ext table-responsive">
                            <form action="/" id="check_all_table" method="post">
                                <table id="table" class="display dataTable custom-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th><?= $page_lang->method;?></th>
                                            <th><?= $page_lang->description;?></th>
...
<div id="payment" class="modal fade customModel" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold">Filter Payments</h5>
            ...
            <div class="modal-body">
                <form id="filter_data_filter" method="post">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="font-weight-bold text-muted small"><?= $page_lang->description; ?></label>
...
function dataTable()
{
    ...
    var filterData = {};
    if($('#title').val().trim()) { filterData.title = $('#title').val(); }
    if($('#description').val().trim()) { filterData.description = $('#description').val(); } 
    if($('#enabled').val().trim()) { filterData.status = $('#enabled').val(); }
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "/<?= $TYPE; ?>/payment/index_ajax_post",
            "data": postData,
            "type": "POST",
            "dataType": 'json',
            "complete": function () {
        ...
        "columnDefs": [ 
            ...
            },
            {
                ...
                "render": function ( data, type, row ) {
                    // Premium Action Button
                    var toggle_class = (row.status == '1') ? 'fa-toggle-on' : 'fa-toggle-off';
                    ...
                    '<button title="'+row.status+'" class="btn custom-popover-btn btn-status" type="button" data-status="'+row.status+'" data-id="'+row.payment_id+'"><i class="fa '+toggle_class+'"></i></button>'+
                    '<button title="Edit" class="btn custom-popover-btn btn-edit" type="button" data-id="'+row.payment_id+'"><i class="fa fa-pencil-square-o"></i></button>'+
                    '<button title="View" class="btn custom-popover-btn btn-view" data-id="'+row.payment_id+'"><i class="fa fa-eye"></i></button>'+
                    '</div>\''+
...
$(document).on('click','.btn-view', function(e){
    ...
    window.location.href = base_url+"/admin/payment/view/"+uid; 
});
...
function status_change(id,status)
{
    ...
    $.ajax({
            url         :   '<?php echo base_url();?>admin/payment/status_change',
            type        :   'post',
...
function delete_post(id)
{
    ...
    $.ajax({
            url         :   '<?php echo base_url();?>admin/payment/del',
            type        :   'post',
            enctype     :   'multipart/form-data',
            dataType    :   "json",  
            data        :   postData,
...
function checkAll() { 
    ...
    if(status != '' && paymentId.length > 0){
       checkall_status_change(paymentId,status);
    }
}
...
function checkall_status_change(id,status)
{
    ...
    $.ajax({
            url         :   '<?php echo base_url();?>admin/payment/checkall_status_change',
            type        :   'post',            
            dataType    :   "json",  
            data        :   postData,
            success     :   function(data){
