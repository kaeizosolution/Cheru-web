...
<style>
    ...
    .btn-status { background-color: #10b981; } /* Green */
            ...
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-header-custom">
                        <h5 class="card-title-custom">Currency List</h5>
                        <div class="topAction-btn" style="display:flex; gap:10px;">
                            <a href="/<?= $TYPE?>/currency/add" class="btn-custom btn-add" title="Add New">
                            ...
                            <table id="table" class="display dataTable custom-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th><?= $page_lang->country; ?></th>
                                        <th><?= $page_lang->symbol; ?></th>
                                        <th><?= $page_lang->iso_code; ?></th>
                                        <th><?= $page_lang->rate; ?></th>
                                        <th><?= $page_lang->last_updated; ?></th>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold">Filter Currencies</h5>
...
function dataTable()
{
    ...
    table = $('#table').DataTable({
        ...
        "ajax": {
            "url": "/<?= $TYPE; ?>/currency/index_ajax_currency",
            "data": postData,
            "type": "POST",
            "dataType": 'json',
            "complete": function () {
                ...
                // CRITICAL FIX: Initialize Popovers AFTER table loads
                $('[data-toggle="popover"]').popover({
                    html: true,
                    trigger: 'manual',
                    container: 'body' // Prevents cutting off
                });
            },
        },
        "language": {
        ...
        "columnDefs": [ 
            {
                ...
                "render": function ( data, type, row ) {
                    // PHP returns numeric: 1 = active, 0 = inactive
                    if(parseInt(data) === 1) {
            ...
            },
            {
                ...
                "render": function ( data, type, row ) {
                    // 1 = active (toggle-on icon), 0 = inactive (toggle-off icon)
                    var toggle_class = (parseInt(row.status) === 1) ? 'fa-toggle-on text-success' : 'fa-toggle-off text-secondary';
                    
                    // Premium Action Popover Content — pass numeric status
                    var contentHTML = '<div style="display:flex; gap:5px;">'+
                    '<button title="Toggle Status" class="btn custom-popover-btn btn-status" type="button" data-status="'+row.status+'" data-id="'+row.currency_id+'"><i class="fa '+toggle_class+'"></i></button>'+
                    '<button title="Edit" class="btn custom-popover-btn btn-edit" type="button" data-id="'+row.currency_id+'"><i class="fa fa-pen"></i></button>'+
                    '</div>';

                    var actionBtn = '<button type="button" class="btn btn-premium-action" data-toggle="popover" data-placement="bottom" data-content=\''+contentHTML+'\'>'+
        ...
        // Re-init popovers on table redraw (pagination/sort)
...
$(document).on('click','.btn-edit', function(e){
    ...
    var url="/<?php echo $TYPE ?>/currency/update/"+uid+"";
    url_new_tab(e,url);
});
...
$(document).ready(function() {
    $(document).on('click', '.btn-status', function (e) {
        ...
        var status = $(this).attr('data-status'); // numeric: 1=active, 0=inactive
        var postData = {"<?= $csrf->name;?>" : "<?= $csrf->hash; ?>"};
        postData.id = id;
        postData.status = status; // send numeric value
        ...
        .then((willDelete) => {
            if (willDelete) {
                 $.ajax( {
                     url:'/<?= $TYPE; ?>/currency/status_change/',
                     type:'post',
                     dataType:"json",  
                     data: postData,
