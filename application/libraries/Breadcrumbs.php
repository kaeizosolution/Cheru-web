...
class Breadcrumbs {
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
                    ...
                    if($key == 'Home'){
                         $output .= $this->crumb_open.'<a href="' . $crumb['href'] . '"><i class="fa fa-home" aria-hidden="true"> ' . $crumb['page'] . '</i></a> '.$this->crumb_divider.$this->crumb_close;
                    }else{
	...
	function show_new($crumbs)
	{
		...
		if ($this->breadcrumbs) {
            ...
            $output=' <nav aria-label="breadcrumb"><ul class="breadcrumb text-uppercase">';
			// set output variable
			#$output .= $this->tag_open;
