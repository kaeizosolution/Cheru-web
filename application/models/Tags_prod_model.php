...
class Tags_prod_model extends CI_Model{
	...
	function find_active(){
		$this->db->select('c.*');
		$this->db->join('ec_categories_pord c','pc.category_id=c.id');
		$this->db->join('products p','pc.product_id=p.id');
		$this->db->where('c.enabled',1);
		$this->db->where('p.enabled',1);
		$this->db->group_by('pc.category_id');
		$this->db->order_by('c.name','asc');
		$categories = $this->db->get('ec_posts_categories pc')->result_array();
		return $categories;
	}
